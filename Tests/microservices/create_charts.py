...
def create_performance_charts(csv_file, metric_name, output_dir):
    ...
    df["summed_value"] = df.iloc[:, 1:7].sum(axis=1)

    plt.figure(figsize=(12, 6))
    plt.plot(df['sequence'], df["summed_value"], linewidth=1.5, color='#2563eb')

    plt.xlabel('Czas (s)', fontsize=12)
    plt.ylabel(metric_name, fontsize=12)
    plt.title(f'{metric_name} - Wszystkie mikroserwisy', fontsize=14, fontweight='bold')
    plt.grid(True, alpha=0.3)

    avg_value = df["summed_value"].mean()
    max_value = df["summed_value"].max()
    min_value = df["summed_value"].min()
